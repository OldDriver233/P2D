...
void stiffness_anode::generate(Eigen::Ref<MatrixXd> u, Eigen::Ref<MatrixXd> du, std::vector<Eigen::Triplet<double>> &t, Eigen::Ref<VectorXd> res, bool is_first_step) {
    int dim = 1, n = 2;
    ...
    double eff_mat = std::pow(constant::epsilon_e_an, constant::bruggeman);
    double eff_mat_s = std::pow(constant::epsilon_s_an, constant::bruggeman);
    ...
    for(int i = 0; i < this->surface_an_sep; i++) {
        ...
        for(int j = 0; j < n; j++) {
            ...
            double uoc_v = uoc(e_a(j), 1);
            double d_uoc_v = d_uoc(e_a(j), 1);
