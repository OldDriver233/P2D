...
void stiffness_cathode::generate(const Eigen::Ref<MatrixXd> &u, const Eigen::Ref<MatrixXd> &du, std::vector<Eigen::Triplet<double>> &t, Eigen::Ref<VectorXd> res, bool is_first_step) {
    const int dim = 1, n = 2;
    ...
    const int simd_size = elem_cnt - this->surface_ca_sep + 1;
    ...
    for(int i = this->surface_ca_sep; i <= elem_cnt; ++i) {
        ...
        double j0_v = j0<2>(u_ptr[dof_cnt + idx], u_ptr[2 * dof_cnt + 3 * dof_cnt_eff + idx]);
        double d_j0_a_v = d_j0_a<2>(u_ptr[dof_cnt + idx], u_ptr[2 * dof_cnt + 3 * dof_cnt_eff + idx]);
        double d_j0_e_v = d_j0_e<2>(u_ptr[dof_cnt + idx], u_ptr[2 * dof_cnt + 3 * dof_cnt_eff + idx]);
        double uoc_v = uoc<2>(u_ptr[2 * dof_cnt + 3 * dof_cnt_eff + idx]);
        double d_uoc_v = d_uoc<2>(u_ptr[2 * dof_cnt + 3 * dof_cnt_eff + idx]);
        double bv_v = bv(u_ptr[2 * dof_cnt + idx] - u_ptr[i] - uoc_v);
        double d_bv_v = d_bv(u_ptr[2 * dof_cnt + idx] - u_ptr[i] - uoc_v);

        kqp[i - this->surface_ca_sep] = -j0_v * d_bv_v * (-1) * c_max * ce_root / j_ref;
        kqc[i - this->surface_ca_sep] = -d_j0_e_v * bv_v * c_max * ce_root / j_ref;
        kqs[i - this->surface_ca_sep] = -j0_v * d_bv_v * c_max * ce_root / j_ref;
        kqa[i - this->surface_ca_sep] = -d_j0_a_v * bv_v * c_max * ce_root / j_ref - j0_v * d_bv_v * (-1) * d_uoc_v * c_max * ce_root / j_ref;
        rq[i - this->surface_ca_sep] = u_ptr[2 * dof_cnt + dof_cnt_eff + idx] - j0_v * bv_v * c_max * ce_root / j_ref;
