...
double particle_solver::calc(Eigen::Ref<MatrixXd> c_s, const Eigen::Ref<MatrixXd> &u, int pt_size, int an, int ca, int type, int temp) {
    ...
    if(type == 1) {
        ...
        for(int i = ca; i < pt_size; i++) {
            if constexpr (!constant_matrix) {
                ...
                j_coeff = solver_A.solve(pre_j_coeff);
                int idx = i - ca + an + 1;
                MatrixXd Bc_s = assembled_B / this->step_control->dt_now * last_cs.block(idx * particle_dof_size, 0, particle_dof_size, 1);
...
template double particle_solver::calc<true>(Eigen::Ref<MatrixXd>, const Eigen::Ref<MatrixXd>&, int, int, int, int, int);
template double particle_solver::calc<false>(Eigen::Ref<MatrixXd>, const Eigen::Ref<MatrixXd>&, int, int, int, int, int);