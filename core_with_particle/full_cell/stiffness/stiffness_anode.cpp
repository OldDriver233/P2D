...
void stiffness_anode::generate(const Eigen::Ref<MatrixXd> &u,
    ...
    std::vector<double> arr_kappa(n * mesh.anode_elements.size());
    std::vector<double> arr_d_kappa(n * mesh.anode_elements.size());
    std::vector<double> arr_d_eff(n * mesh.anode_elements.size());
    ...
    MatrixXd vars(n * mesh.anode_elements.size(), 2);
