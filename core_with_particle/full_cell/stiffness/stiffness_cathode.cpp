...
void stiffness_cathode::generate(const Eigen::Ref<MatrixXd> &u, 
    ...
    for(int i = this->surface_ca_sep - this->surface_an_coll; i <= elem_cnt; ++i) {
        const int idx = i - this->surface_ca_sep + this->surface_an_coll;
        ...
        kqp[idx] = -j0_v * d_bv_v * (-1) * c_max * ce_root;
        kqc[idx] = -d_j0_e_v * bv_v * c_max * ce_root;
        kqs[idx] = -j0_v * d_bv_v * c_max * ce_root;
        kqq[idx] = j_ref - (d_j0_a_v * bv_v * c_max * ce_root - j0_v * d_bv_v * d_uoc_v * c_max * ce_root) * dc_ssdj;
        rq[idx] = u_ptr[2 * dof_cnt + dof_cnt_eff + idx] * j_ref - j0_v * bv_v * c_max * ce_root;
    }
